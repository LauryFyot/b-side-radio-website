...
-- Force public inserts into approved status (immediate publish)
...
join public.permissions p on p.slug in (
  ...
  'settings.manage','now_playing.manage'
)
where r.slug = 'admin'
on conflict do nothing;
...
-- ----------
-- Helper RPC to grant first admin
-- ----------
...
begin
  if not public.has_role('super_admin') then
    -- bootstrap: allow first call if no super_admin exists yet
    if exists (
      select 1
      from public.user_roles ur
      join public.roles r on r.id = ur.role_id
  ...
  elsif not public.has_role('super_admin') then
    raise exception 'Only super_admin can grant roles';
  end if;
